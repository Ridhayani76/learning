                    ...
                    <tbody>
                        @foreach($practice->members as $i => $member)
                            <tr>
                                ...
                                <td>
                                    <h6>{{$member->student->name}} <span class="text-info">{{$member->student->id == auth()->user()->student->id ? '(Anda)' : ''}}</span></h6>
                                    <span class="d-flex align-items-center text-muted" style="letter-spacing: 1.2px; font-weight: 500;">
                                        <ion-icon name="card-outline" class="mr-2" style="font-size: 14px"></ion-icon> {{$member->student->nim}}
                                    </span>
                                </td>
                                <td>
                                    <h6>Kelas</h6>
                                    <span class="d-flex align-items-center text-muted">
                                        <ion-icon name="layers-outline" class="mr-2" style="font-size: 14px"></ion-icon> Kelas {{$member->student->classroom->name}}
                                    </span>
                                </td>
                                <td>
                                    <h6>Status</h6>
                            ...
                            <select name="student_id" id="student_id" class="form-control">
                                @foreach(\App\Classroom::orderBy('name', 'asc')->get() as $classroom)
                                    @foreach($classroom->students as $student)
                                        <option value="{{$student->id}}">{{$classroom->name}} / {{$student->nim}} / {{$student->name}}</option>
                                    @endforeach
                                @endforeach
                            </select>
                        </div>
